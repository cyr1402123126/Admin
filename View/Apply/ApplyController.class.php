...
class ApplyController extends CommonController{
	...
	public function preview(){
		...
		// if($_GET['i']>=0){
		// 	$arrs=explode(';',$arr[$zd]);
		// 	$imgurl='http://www.crm.com'.ltrim($arrs[$_GET['i']],'.');
		// 	echo "<center><img align='left' src=".$imgurl." height='580px' style='margin-top:80px;'></center>";
		// }else{
		// 	if($arr["{$zd}"]){
	...
	public function yewutwo(){
		...
		// var_dump($data);
		$datass=M("cashfiles")->where("aid={$_GET['id']}")->Field("id_card,yz_card,poid_card,house_certificate,credit,flow,b_license")->find();
		    ...
		    foreach($_FILES as $key=>$value){
	          if(count($_FILES[$key]) == count($_FILES[$key],1)){
	          	...
	          	if($key=="poid_card"){
	          		$info = $upload->uploadOne($_FILES[$key]);
	          		if($info){
	          			$data['poid_card']='./Uploads/'.$info['savepath'].$info['savename'];
	          		}
	          	}
	          	if($key=="b_license"){
	          		$info = $upload->uploadOne($_FILES[$key]);
	...
	public function investigate(){
		if(IS_POST){
			...
			// if($datass['house_photo1']){
			// 	$arr=explode(';', $datass['house_photo1']);
			// 	$this->assign('hcount',$arr);
			// }
			// if($datass['company_photo']){
			// 	$arr=explode(';', $datass['company_photo']);
			// 	$this->assign('ccount',$arr);
			// }
	...
	public function sign(){
		if(IS_POST){
			...
			$datass=M("cashfiles")->where("aid={$_GET['id']}")->Field("loan_contract,iou,letter_commitment,mail_list,sign_scene")->find();
